...
public class SecondCrewGui extends ConfigurableGui {


    public SecondCrewGui(@NotNull Player player) {
    ...
    private void putrewards(){
        List<ItemStack> items = (List<ItemStack>) ConfigurationHandler.getConfig().getList("rewards.2");
        for(ItemStack item : items){
            Icon icon = new Icon(item);
            icon.onClick(e -> {
                Util.giveOrDrop(player, item);
                ConfigurationHandler.getConfig().getList("rewards.2").remove(item);
    ...
    private void putContent() {
        ...
        addConfigIcon("save-item").onClick(e -> {
            ...
            ConfigurationHandler.getConfig().set("rewards.2", items);
            ConfigurationHandler.saveConfig();
        });
