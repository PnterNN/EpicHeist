...
import mc.pnternn.epicheist.guis.AdminPanelLocationsGui;
...
public class FirstCrewGui extends ConfigurableGui {

    public FirstCrewGui(@NotNull Player player) {
    ...
    private void putrewards(){
        List<ItemStack> items = (List<ItemStack>) ConfigurationHandler.getConfig().getList("rewards.1");
        for(ItemStack item : items){
            Icon icon = new Icon(item);
            icon.onClick(e -> {
                Util.giveOrDrop(player, item);
                ConfigurationHandler.getConfig().getList("rewards.1").remove(item);
    ...
    private void putContent() {
        ...
        addConfigIcon("save-item").onClick(e -> {
            ...
            ConfigurationHandler.getConfig().set("rewards.1", items);
            ConfigurationHandler.saveConfig();
        });
